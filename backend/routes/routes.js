...
router.get('/notes',async(req,res)=>{
  ...
  if(notes){
    res.status(200).json({msg:"success",notesData:notes,Total:totalNotes});
  }else{
...
router.get('/verify-email/:id',async(req,res)=>{
  ...
  const verified=await signup.updateOne({_id:id},{$set:{is_verify:true}});
...
router.post("/send-reset-password-link",async(req,res)=>{
 ...
 const token=uuidv4()
  ...
  const add_token=signup.updateOne({email:email},{$set:{token:token}}).then((response)=>{
...
router.post('/forget-password/:token',async(req,res)=>{
    ...
    const updatedpassword=signup.updateOne({token:token},{$set:{password:newhashPassword,token:''}}).then(()=>{
...
router.post('/upload-notes',upload.fields([{name:'image',maxCount:1},{name:'notesfile',maxCount:1}]),(req,res)=>{
...
router.delete('/delete-notes/:id',(req,res)=>{
  ...
  const deletedNotes=notesModel.deleteOne({_id:Id}).then(()=>{
    res.status(200).json({msg:'success'})
  }).catch((err)=>{
    res.status(501).json({msg:'error'})
  })
...
router.delete('/delete-userquery/:id',(req,res)=>{
  ...
  const deletedQuery=contactDetails.deleteOne({_id:Id}).then(()=>{
    res.status(200).json({msg:'success'})
  }).catch((err)=>{
    res.status(501).json({msg:'error'})
  })
